...
class TagTool(CommandLineInterface):
	...
	#chatNamespace = socketIO.define(ChatNamespace, '/test')
        #with SocketIO('localhost/test', 5000, ChatNamespace) as socketIO:
        #    socketIO.emit('1')
	...
	#socketIO.define(TestNamespace, '/test')
        ...
        #tagstr = tag.identifier.encode("hex").upper()
        if tag.__class__.__name__ ==  "Type4ATag":
            print ("type4")
            print (tag._dep.fwt)
            print (str(hex(struct.unpack('<I', struct.pack('<f',tag._dep.fwt))[0])))
            tagstr = str(hex(struct.unpack('<I', struct.pack('<f',tag._dep.fwt))[0]))
        else:
            tagstr = tag.identifier.encode("hex").upper()
            
        socketIO.emit('sendTagId',{'data':tagstr})
        print(tag)
        time.sleep(1)
